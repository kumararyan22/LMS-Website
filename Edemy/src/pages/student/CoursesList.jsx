...
const CoursesList = () => {
  ...
  const [filteredredBoursew , setFilteredCourses] = useState([]);
  ...
  <>
    <div className='relative md:px-36 px-8 pt-20 text-left'>
      ...
      { input && <div className='inline-flex items-center px-4 py-2 gap-4 border mt-8 -mb-8 text-gray-600'>
        <p>{input}</p>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 my-16 px-2 md:p-0'>
          {filteredredBoursew.map((course,index)=> <CourseCard key={index} course={course}/>)}
        </div>
