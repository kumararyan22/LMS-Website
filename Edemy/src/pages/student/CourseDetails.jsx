...
const CourseDetails = () => {
  ...
  const {allCourses,calculateRating,calculateChapterTime,calculateCourseDuration,calculateNumOfLectures,currency} = useContext(AppContext);
  ...
  return courseData ? (
    ...
    <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
          ...
          <div className='pt-5'>
            {
              ...
              (
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded '>
                  ...
                  <div className={`overflow-hidden duration-300 transition-all ${openSections[index] ? 'max-h-96' : 'max-h-0' }`}>
                    <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                      {chapter.chapterContent.map((lecture,i)=> (
                          <li key={i} className='flex items-start py-1 gap-2'>
                            <img src={assets.play_icon} alt='play icon' className='w-4 h-4 mt-1 '/>
                            <div className='flex w-full items-center justify-between text-gray-800 text-xs md:text-default'> 
      ...
      {/* right colum */}
      <div className='max-w-course-card z-10 shadow-custom-card rounded-t-lg md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
